...
#ifndef OSGTRANSPARENCY_UTIL_STATS_H
#define OSGTRANSPARENCY_UTIL_STATS_H
...
public:
    ...
    /* Constructor */
public:
    Stats(const std::string& name, const unsigned int maxFrames = 5)
        : _name(name)
        , _stats(maxFrames)
        , _firstFrame(0)
        , _lastFrame(0)
    ...
    /* Member functions */
public:
    bool setMeasure(const int frameNumber, const std::string& name,
                    const double value)
    {
    ...
    bool incrementMeasure(const int frameNumber, const std::string& name,
                          const double value)
    {
        assert(_clearedFrames.find(frameNumber) == _clearedFrames.end());
        MeasureMap* measures = getFrameMeasures(frameNumber);
    ...
    void setMeasure(const std::string& name, const double value)
    {
        MeasureMap& measures = getLastFrameMeasures();
        measures.insert(std::make_pair(name, value));
    }

    void incrementMeasure(const std::string& name, const double value)
    {
        double& measure = getLastFrameMeasures()[name];
        measure += value;
    }

    bool getMeasure(const int frameNumber, const std::string& name,
                    double& value) const
    {
