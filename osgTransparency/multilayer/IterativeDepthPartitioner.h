...
#ifndef OSGTRANSPARENCY_MULTILAYER_ITERATIVEDEPTHPARTITIONER_H
#define OSGTRANSPARENCY_MULTILAYER_ITERATIVEDEPTHPARTITIONER_H
...
class IterativeDepthPartitioner : public DepthPartitioner
{
public:
    /*--- Public constructors/destructor ---*/

    IterativeDepthPartitioner(const Parameters& parameters);

    ~IterativeDepthPartitioner();
    ...
    void updateShaderPrograms(const ProgramMap& extraShaders) final;

    void computeDepthPartition(MultiLayerDepthPeelingBin* bin,
                               osg::RenderInfo& renderInfo,
                               osgUtil::RenderLeaf*& previous) final;

    osg::ref_ptr<osg::TextureRectangle>* getDepthPartitionTextureArray() final
    {
...
private:
    ...
    osg::ref_ptr<osg::TextureRectangle> _countTextures[8];
